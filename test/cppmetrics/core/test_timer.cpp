...
TEST(timer, timerContextTest) {
	...
	std::cout << "         mean rate = " << (timer.getMeanRate()) << " calls per 1 sec" << std::endl;
	std::cout << "     1-minute rate = " << (timer.getOneMinuteRate()) << " calls per 1 sec" << std::endl;
	std::cout << "     5-minute rate = " << (timer.getFiveMinuteRate()) << " calls per 1 sec" << std::endl;
	std::cout << "    15-minute rate = " << (timer.getFifteenMinuteRate()) << " calls per 1 sec" << std::endl;
	std::cout << "               min = " << (snapshot->getMin()) << " ns " << std::endl;
	std::cout << "               max = " << (snapshot->getMax()) << " ns " << std::endl;
	std::cout << "              mean = " << (snapshot->getMean()) << " ns " << std::endl;
	std::cout << "            stddev = " << (snapshot->getStdDev()) << " ns " << std::endl;
	std::cout << "            median = " << (snapshot->getMedian()) << " ns " << std::endl;
	std::cout << "              75% <= " << (snapshot->get75thPercentile()) << " ns " << std::endl;
	std::cout << "              95% <= " << (snapshot->get95thPercentile()) << " ns " << std::endl;
	std::cout << "              98% <= " << (snapshot->get98thPercentile()) << " ns " << std::endl;
	std::cout << "              99% <= " << (snapshot->get99thPercentile()) << " ns " << std::endl;
	std::cout << "            99.9% <= " << (snapshot->get999thPercentile()) << " ns " << std::endl;
	ASSERT_LE(25, static_cast<int>(timer.getMeanRate()));
	ASSERT_GE(55, static_cast<int>(timer.getMeanRate()));
	ASSERT_LE(boost::chrono::duration_cast<boost::chrono::nanoseconds>(boost::chrono::milliseconds(8)).count(),
			static_cast<int>(snapshot->getMin()));
	ASSERT_GE(boost::chrono::duration_cast<boost::chrono::nanoseconds>(boost::chrono::milliseconds(45)).count(),
