...
void GraphiteReporter::reportTimer(const std::string& name,
		core::TimerPtr timer, boost::uint64_t timestamp) {
...
void GraphiteReporter::reportMeter(const std::string& name,
		core::MeteredPtr meter, boost::uint64_t timestamp) {
...
void GraphiteReporter::reportHistogram(const std::string& name,
	...
	sender_->send(prefix(name, "count"), format(histogram->getCount()), timestamp);
	sender_->send(prefix(name, "getMax"), format(snapshot->getMax()), timestamp);
...
void GraphiteReporter::reportCounter(const std::string& name,
	...
	sender_->send(prefix(name, "count"), format(counter->getCount()), timestamp);
}

void GraphiteReporter::reportGauge(const std::string& name,
		core::GaugePtr gauge, boost::uint64_t timestamp) {
