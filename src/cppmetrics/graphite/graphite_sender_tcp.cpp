...
void GraphiteSenderTCP::send(const std::string& name, const std::string& value, boost::uint64_t timestamp) {
	if (!connected_) {
	...
	boost::asio::write(*socket_, boost::asio::buffer(graphite_str, graphite_str.size()));
}
