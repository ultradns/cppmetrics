...
static const uint64_t TICK_INTERVAL = Clock::duration(boost::chrono::seconds(5)).count();
...
Meter::Impl::Impl(boost::chrono::nanoseconds rate_unit) :
				...
				last_tick_(
						boost::chrono::duration_cast<boost::chrono::nanoseconds>(start_time_.time_since_epoch()).count()),
...
double Meter::Impl::getMeanRate() {
	...
	if (c > 0) {
		boost::chrono::nanoseconds elapsed = boost::chrono::duration_cast<boost::chrono::nanoseconds>(
...
void Meter::Impl::tickIfNecessary() {
	...
	boost::uint64_t cur_tick =
			boost::chrono::duration_cast<boost::chrono::nanoseconds>(Clock::now().time_since_epoch()).count();
