...
const Clock::duration ExpDecaySample::RESCALE_THRESHOLD(boost::chrono::hours(1));
...
void ExpDecaySample::update(boost::int64_t value, const Clock::time_point& timestamp) {
	boost::lock_guard<boost::mutex> rlock(mutex_);
	rescaleIfNeeded(timestamp);
	boost::random::uniform_real_distribution<> dist(0, 1);
	boost::chrono::seconds dur = boost::chrono::duration_cast<boost::chrono::seconds>(timestamp - start_time_);
	double priority = 0.0;
	do {
	...
	if (count <= reservoir_size_) {
		...
		if (first < priority && values_.insert(std::make_pair(priority, value)).second) {
