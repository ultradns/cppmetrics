...
private:
	// Old C++98 style enum.
	enum MetricType {
		GaugeType = 0, CounterType, HistogramType, MeterType, TimerType, TotalTypes
	};
	...
	bool addMetric(MetricSet& metric_set, const std::string& name, MetricPtr metric);
...
bool MetricRegistry::Impl::isInstanceOf(const MetricPtr& metric_ptr) const {
	boost::shared_ptr<MetricClass> stored_metric(boost::dynamic_pointer_cast<MetricClass>(metric_ptr));
...
bool MetricRegistry::Impl::addMetric(MetricSet& metric_set, const std::string& name, MetricPtr new_metric) {
	...
	throw std::invalid_argument(name + " already exists as a different metric.");
}

#if 0
template<typename MetricClass>
boost::shared_ptr<MetricClass> MetricRegistry::Impl::getOrAdd(const std::string& name) {
	boost::upgrade_lock<boost::shared_mutex> read_lock(metrics_mutex_);
	MetricSet::iterator itt(metric_set_.find(name));
	if (itt != metric_set_.end()) {
		if (!isInstanceOf<MetricClass>(itt->second)) {
			throw std::invalid_argument(
					name + " already exists as a "
					+ typeid(*itt->second).name());
		}
		return boost::static_pointer_cast<MetricClass>(itt->second);
	} else {
		boost::shared_ptr<MetricClass> newMetric(
				boost::make_shared<MetricClass>());
		boost::upgrade_to_unique_lock<boost::shared_mutex> upgrade_lock(
				read_lock);
		metric_set_.emplace(name, newMetric);
		return newMetric;
	}
}
#else
MetricPtr MetricRegistry::Impl::getOrAdd(MetricType metric_type, const std::string& name) {
...
template<typename MetricClass>
std::map<std::string, boost::shared_ptr<MetricClass> > MetricRegistry::Impl::getMetrics(
